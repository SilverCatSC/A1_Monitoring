...
class ListingPlacementIdentity(Base):
    """Verified platform-specific unique_id bound to one registry listing."""

    __tablename__ = 'listing_placement_identities'

    id: Mapped[str] = mapped_column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    listing_id: Mapped[str] = mapped_column(ForeignKey('listings.id'), nullable=False, index=True)
    source: Mapped[EngineType] = mapped_column(Enum(EngineType), nullable=False)
    placement_id: Mapped[str] = mapped_column(String(22), nullable=False)
    evidence: Mapped[str] = mapped_column(Text, nullable=False)
    verified_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)

    __table_args__ = (
        UniqueConstraint('source', 'placement_id', name='uq_placement_identity_source_id'),
        UniqueConstraint('source', 'listing_id', name='uq_placement_identity_source_listing'),
    )
