...
1. Завершить перенос отчётов и AI-пакетов с `latest` на закрытый `cycle_id`.
   Базовый ledger, связь стадий и immutable roster manifest уже реализованы.
...
4. Сделать отсутствие изображения явным техническим пропуском.
5. Добавить HTML-фикстуры CAPTCHA, снятой карточки, редиректа, пустой выдачи и нового формата Auto.ru.
6. Только после этого сравнить локальную модель с API на размеченной выборке.
