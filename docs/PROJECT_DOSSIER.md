...
1. Добавить постоянный `vehicle_id/offer_id`, независимый от VIN, цены и URL.
2. Разделить результат страницы и полноту всего фильтра: найденное сохранять даже при последующей ошибке.
3. Сделать отсутствие изображения явным техническим пропуском.
4. Добавить HTML-фикстуры CAPTCHA, снятой карточки, редиректа, пустой выдачи и нового формата Auto.ru.
5. Только после этого сравнить локальную модель с API на размеченной выборке.
