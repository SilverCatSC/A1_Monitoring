...
- Старый путь `/Users/filaret/Desktop/a1_search_monitor_noapi_product` сохранён как символическая ссылка для совместимости старых абсолютных путей.
- Python импортирует приложение из новой папки.
- Docker-приложение и PostgreSQL отвечают на локальном `/api/v1/ready`.
- 214 тестов проходят; Ruff и проверка внутренних Markdown-ссылок проходят.
...
- Машины без VIN могут получить одинаковый составной ключ и слиться.
- Текущий публичный HTML-экспорт не является безопасным обезличивателем.
...
- Запускатели Mac/Windows пока не образуют единого возобновляемого coordinator.
- Проверка Windows и полный живой no-VPN прогон не приняты.
...
1. Ввести единый `cycle_id` для импорта, каталогов, поиска, карточек, аудитов и AI.
2. Добавить постоянный `vehicle_id/offer_id`, независимый от VIN, цены и URL.
3. Разделить результат страницы и полноту всего фильтра: найденное сохранять даже при последующей ошибке.
4. Сделать строгий итог обязательных этапов: ошибка карточек не может дать `completed`.
5. Заменить `latest`-сборку на атомарный immutable manifest одного цикла.
6. Сделать отсутствие изображения явным техническим пропуском.
7. Добавить HTML-фикстуры CAPTCHA, снятой карточки, редиректа, пустой выдачи и нового формата Auto.ru.
8. Только после этого сравнить локальную модель с API на размеченной выборке.
...
Публичный GitHub может содержать код и безопасную документацию. Рабочий отчёт для отдела продаж должен оставаться защищённым; текущий `export_public_report.py` доработать до allowlist/redaction до любой публикации.
