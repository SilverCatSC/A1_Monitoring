...
Видимый Mac host-runner по умолчанию ждёт до 180 секунд, если Auto.ru после
одного обычного обновления всё ещё показывает интерактивную CAPTCHA. Оператор
проходит её сам в открытой приватной вкладке; ход проверки показывает
«Требуется действие». Время можно изменить параметром
`--captcha-wait-seconds 0..600`, где `0` отключает ожидание. Это действует
только на текущее посещение: новые приватные контексты следующих фильтров и
карточек не наследуют пройденную проверку. Avito HTTP 429 этим не устраняется.
