...
Отдельный внешний gate для Avito: [разрешённый автономный доступ к данным
объявлений](TASK_AVITO_UNATTENDED_ACCESS_2026-09-25.md). Он не заменяется
автоматическим решением CAPTCHA; без подтверждённых прав и live-test Avito
остаётся непроверенным, а M7 не закрывается.
...
Дополнение от 24.09.2026: ограниченная сверка ID фидов с новыми карточками
каталога [подключена к циклу в коде](PLACEMENT_CYCLE_INTEGRATION_2026-09-24.md),
но выключена по умолчанию. Проверены загрузка трёх реальных фидов и офлайн-
сценарии перевыкладки; прямое открытие живых карточек в новом этапе и ручная
сверка контрольной выборки ещё не выполнялись. Это не закрывает M7.
