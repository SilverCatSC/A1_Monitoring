...
Срез исходников 14.09.2026, приложение 0.9.0. Карта описывает существующий код;
...
flowchart TD
    ...
    B --> C[Чтение Monitoring и проверка входных данных]
    C --> D{Импорт принят?}
    D -- Нет --> E[Сохранить диагностику и предыдущий реестр; остановить цикл]
    D -- Да --> F[Обновить реестр и назначения фильтров]
    F --> G[Открыть служебный видимый Chrome]
    G --> H[Каталоги продавца: Auto.ru cars и LCV; Avito]
    H --> I{Текущий ID объявления подтверждён?}
    I -- Нет --> J[Очередь сверки; ограниченная проверка старой ссылки]
    I -- Да --> K[Поиск по назначенным фильтрам до трёх страниц]
    J --> L[Неподтверждённые ожидания: technical_error]
    K --> M{Выдача разобрана полностью в нужном окне?}
    M -- Нет --> N[technical_error; при блокировке остановить площадку]
    M -- Да --> O{ID найден?}
    O -- Да --> P[found: страница, позиция, цена и снимок]
    O -- Нет --> Q[Непоказ в проверенном окне; серия достоверных пропусков]
    P --> R[Последовательная проверка прямых карточек]
    Q --> R
    N --> R
    L --> R
    R --> S[Сохранить статусы прямых карточек и снимки]
    S --> T[Отдельный аудит сайта A1Auto]
    T --> U[Отдельный аудит таблицы маркетинга]
    U --> V[Собрать latest-файлы и последние scan runs в AI-пакет]
    V --> W[Разбить пакет по автомобилю и снимку]
    W --> X[Hermes Data, Vision; затем краткая Synthesis]
    X --> Y[Проверить JSON и связь итогов с малыми findings]
    Y --> Z[Ouroboros: аудит staged-отчёта]
    Z --> AA[Сохранить AI-файлы; оператор смотрит отчёт и замечания]
```

Ошибки, блокировки и неполное покрытие не гарантированно попадают в один общий
статус полного сценария: это один из дефектов текущего координатора. Порядок
аудитов сайта и маркетинга выше соответствует оболочкам `run_full_monitoring_*`.
...
Автомобиль находится по хешу VIN; без VIN — по комбинации внутренних полей,
марки, модели, поколения и года. Здесь возможна коллизия двух одинаковых машин.
Пустой статус в Monitoring тоже считается активным. Новые ссылки сверяются с
...
`build_live_agent_packet.py` объединяет последнюю статистику сканов, `latest.json`
аудита маркетинга и сайта. `build_ai_work_units.py` формирует один пакет на машину
