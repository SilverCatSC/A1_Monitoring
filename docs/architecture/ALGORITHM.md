...
того же локального пользователя. Chrome использует CDP на `127.0.0.1:19223` и
отдельный профиль без расширений. Каждое посещение открывается во временном
приватном контексте и закрывает его после проверки. Если endpoint уже доступен,
используется существующий служебный браузер; строгой проверки владельца
endpoint сейчас недостаточно.
...
Первичная связь без VIN требует отдельного подтверждения оператора; после
однозначной ID-привязки новая ссылка может обновиться автоматически, сохраняя историю
...
Перед поиском отдельный этап сравнивает `unique_id` в фиде и карточках дилера и
автоматически обновляет однозначные точные ссылки Monitoring с сохранением старых
URL. Привязка ID к записи автомобиля сохраняется отдельно; VIN не применяется
для замены ссылки. Если такой связи ещё нет, она впервые подтверждается только
совпадением текущего URL, точного ID в фиде и активной карточке с доказательством.
Неполная сверка не мешает искать уже подтверждённые ссылки: остальные
машины получают `review_required`. Полностью недоступный ID-этап останавливает
поиск. Подробнее — [контракт автоактуализации](../production/TASK_AUTOMATIC_LINK_SYNC_2026-09-25.md).
