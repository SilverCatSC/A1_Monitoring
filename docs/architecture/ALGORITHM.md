...
Срез исходников 14.09.2026, приложение 0.12.0. Карта описывает существующий код;
...
flowchart TD
    ...
    R --> T[Скриншот + hash manifest точной карточки]
    S --> T
    P --> T
    N --> T
    T --> U[Прямая карточка: screenshot + manifest до статуса]
