...
Срез исходников 14.09.2026, приложение 0.10.0. Карта описывает существующий код;
...
flowchart TD
    ...
    O -- Да --> Q{ID найден?}
    Q -- Да --> R[found: страница, позиция, цена и снимок]
    Q -- Нет --> S[Непоказ в проверенном окне; серия достоверных пропусков]
    R --> T[Последовательная проверка прямых карточек]
    S --> T
    P --> T
    N --> T
    T --> U[Сохранить статусы прямых карточек и снимки]
