...
выгрузка модели → возврат дашборда. Перед моделью требуется не менее
...
Для дополнительной тяжёлой проверки спорных результатов используйте
`-RunHeavyReview`. `-AiProfile heavy` запускает весь основной AI-этап на тяжёлой
модели и нужен только для отдельной диагностики.
