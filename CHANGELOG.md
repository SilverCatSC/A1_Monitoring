...
## M7.13 — точная перевыкладка как действие оператора — 2026-09-25

- Для одного точного ID-кандидата каталог и карточка автомобиля показывают
  старую и новую ссылки, ID, фид и проверенный снимок новой карточки.
- Подтверждение новой ссылки ограничено свежей сверкой и целым evidence;
  похожие объявления по модели не предлагаются вместо точного кандидата.
- После операторского подтверждения журнал показывает `Было → стало`.
  Само подтверждение рабочих ссылок этим изменением не выполнялось.
- Сценарий и границы: `docs/production/TASK_EXACT_ID_REPUBLICATION_2026-09-25.md`.
...
- Дата проверки видна прямо в кратком статусе; повтор модели в названии
  устраняется без изменения исходных данных.
