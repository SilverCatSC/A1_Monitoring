...
- Hermes включён в предфильтровую сверку: сравнивает старую карточку и свежих
  кандидатов по двум снимкам, цене и описанию, записывает оценку и может обновить
  локальную ссылку при единственном строгом совпадении. Изменение журналируется.
- CAPTCHA, HTTP 403 и 429 в поиске и каталоге продавца оставляют управляемую
  вкладку открытой. Цикл ждёт и периодически перезагружает текущую ссылку до
  успешного ответа, поэтому оставшиеся фильтры не пропускаются.
- Повторная навигация Auto.ru больше не обрывает страницу на transient-ошибке
  `Execution context was destroyed`.
- Поиск Auto.ru и Avito завершает фильтр сразу после обнаружения всех ожидаемых
  объявлений. Это сохраняет уже найденные позиции и не открывает лишние страницы,
  способные вызвать CAPTCHA или неопределённый ответ площадки.
- Ouroboros исключён из обязательного мониторинга. Обычный цикл использует лёгкий
  Hermes; тяжёлый Hermes для компактной выборки спорных случаев включается явно
  параметром Windows-запуска `-RunHeavyReview`.
...
- Docker Compose использует transcript-safe вывод `plain`, а `docker.exe`
  обнаруживается также в локальной установке Docker Desktop, даже если её каталог
  отсутствует в `PATH` текущего процесса.
- Полный цикл защищён системным mutex от параллельного запуска. Локальная модель
  обнаруживается по точному `llama-server.exe` и порту, поэтому потерянный PID-файл
  восстанавливается, а остановка не оставляет фоновый процесс.
- CAPTCHA распознаётся до проверки географии, поэтому источник сразу останавливается
  и не маскирует блокирующую страницу ошибкой «Москва, 0 км не подтверждены».
