...
## M7.21 — ограниченное ожидание CAPTCHA в текущей вкладке — 2026-09-25

- Видимый MacBook Chrome после одного refresh Auto.ru сохраняет снимок
  интерактивной CAPTCHA и ждёт штатного действия человека до 180 секунд;
  время настраивается, `0` отключает ожидание. Автоматического решения нет.
- После возврата на запрошенную страницу тот же фильтр продолжает с неё;
  тайм-аут, неподходящий адрес и отсутствие снимка не становятся успешной
  проверкой. Состояние ожидания показано в ходе проверки.
- Контекст пока создаётся на каждое посещение; pending-only resume и живой
  проход Auto.ru/Avito остаются открытыми. Границы — в
  `docs/production/CAPTCHA_RESILIENCE_2026-09-25.md`.

## M7.20 — неизменяемый список незавершённых проверок — 2026-09-25

- Каждый новый цикл записывает `pending_checks.json` по запечатанному roster и
  фактам того же цикла: непроверенные каталоги, поиск, прямые карточки и
  обнаруженные, но не открытые ID-карточки. CAPTCHA, HTTP 429 и отсутствие
  наблюдения остаются pending, а не результатом об отсутствии объявления.
- Количество pending входит в итог цикла; непустой список запрещает статус
  `completed`. Хеш roster проверяется до формирования отчёта.
- Автоматического продолжения и живой приёмки ещё нет. Контракт и ограничения:
  `docs/production/CAPTCHA_RESILIENCE_2026-09-25.md`.

## M7.19 — отмена отсрочки и диагностика CAPTCHA — 2026-09-25

- По замечанию владельца отменена отсрочка: текущие ID-карточки снова
  проверяются до поиска, чтобы CAPTCHA поиска не оставляла их без проверки.
- Auto.ru и Avito разобраны как две разные проблемы: интерактивная
  SmartCaptcha после части цикла и HTTP 429 с сообщением о проблеме IP на
  первом запросе соответственно. Решение и live-приёмка ещё не выполнены.
- Зафиксированы контракт неполных проверок, ручной fallback с согласия
  владельца и отдельная проверка сетевого маршрута:
  `docs/production/CAPTCHA_RESILIENCE_2026-09-25.md`.

## M7.18 — отложенное открытие уже связанной карточки — 2026-09-25

> Исторический эксперимент; отменён в M7.19.

- В ID-этапе текущие URL с сохранённым точным `unique_id`, снова увиденные
  в свежем каталоге дилера, проверяются по содержанию после поиска.
  Новые URL перевыкладок и конфликтные ID открываются до поиска.
- Отложенные карточки отдельно считаются в отчёте; ID-сверка не выдаёт их за
  заново открытые. Это уменьшает нагрузку перед поиском, но не обходит CAPTCHA
  и не решает HTTP 429 Avito на первом запросе.
- Локально `424 passed`; живой повтор пока не выполнялся после блокировок.

## M7.17 — связь по unique_id без VIN и ограниченный CAPTCHA-refresh — 2026-09-25

- Для Auto.ru и Avito подтверждённый `unique_id` сохраняется отдельно от VIN:
  точное совпадение ID в фиде и активной карточке при уже совпадающем URL
  создаёт связь с машиной; дальнейшая перевыкладка использует эту связь.
- Автозамена без сохранённой связи запрещена. При подтверждении точной
  перевыкладки оператором связь также сохраняется; конфликты не переписываются.
- Явная CAPTCHA с успешным HTTP-ответом вызывает не более одного обновления
  страницы. HTTP 429 и повторный challenge остаются неполной проверкой.
- Живая проверка обеих площадок после этой версии ещё должна быть зафиксирована
  отдельно; тесты не доказывают доступность площадок. Цикл
  `816845f2-b19d-4de5-94f1-0e037d53f3a2` зафиксирован как частичный:
  12 ID-связей Auto.ru сохранены, Avito дал HTTP 429, CAPTCHA Auto.ru
  осталась после одного обновления. Детали — в
  `docs/production/TASK_FULL_UNIQUE_ID_CHECK_2026-09-25.md`.

## M7.16 — поиск после частичной ID-сверки — 2026-09-25

- Полная ID-сверка больше не является условием поиска всех машин: после
  автоактуализации проверяются только индивидуально подтверждённые ссылки;
  неопределённые получают `review_required`, цикл остаётся `partial`.
- Если фид или ID-отчёт недоступны, поиск по-прежнему не запускается.
- Лимит прямых ID-проверок поднят с 40 до 80 после живого каталога из 57
  карточек. Уже полученный снимок активной прямой карточки переиспользуется
  вместо второго посещения.
- В контрольном цикле `e245a78b-4f26-43d9-adc1-61fe21253e75` Auto.ru
  действительно перешёл к 9 поисковым фильтрам после ID-этапа; Avito вернул
  HTTP 429, поэтому цикл остался `partial`. Две V-Class карточки активны по
  новым URL, но не встречены в первых трёх страницах фильтра «новые».

## M7.15 — отдельный Chrome без расширений — 2026-09-25

- После первого цикла на приватных контекстах найден конфликт Playwright с
  service worker расширения в старом профиле. Новый служебный Chrome работает
  на CDP `19223` в отдельном профиле с выключенными расширениями; старый
  профиль не удалён. Три локальных приватных подключения к `about:blank`
  прошли, живой цикл требуется повторить.
- Для цикла, где ID-сверка остановила поиск, terminal-runner теперь корректно
  возвращает `partial`, а не сбой форматирования отсутствующих счётчиков.

## M7.14 — ID-ссылки перед поиском, приватный браузер — 2026-09-25

- Полный MacBook-цикл теперь сначала сверяет карточки дилера с фидом и
  автоматически заменяет только однозначные ссылки по точному `unique_id` с
  целым снимком. Старый URL и причина сохраняются в журнале.
- После синхронизации фиксируется roster и только затем начинается поиск;
  неполная ID-сверка оставляет цикл `partial` без поисковых наблюдений.
- Каждое посещение площадки открывается во временном приватном контексте Chrome.
  Это не обходит CAPTCHA и может потребовать действия оператора.
- Контракт и границы: `docs/production/TASK_AUTOMATIC_LINK_SYNC_2026-09-25.md`.
