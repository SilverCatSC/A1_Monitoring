...
| Автономная проверка Avito при HTTP 429: требуемый доступ и границы | [Задача доступа Avito](docs/production/TASK_AVITO_UNATTENDED_ACCESS_2026-09-25.md) |
...
перевыкладка по одному точному `unique_id` теперь может автоматически обновить
ссылку Monitoring до поискового этапа при сохранённой связи
`площадка + unique_id → автомобиль` и целостном evidence — без VIN; неоднозначные
случаи остаются на разборе. Это ещё не заменяет живую приёмку.
...
Видимый Chrome запускается с отдельным служебным профилем, но каждое посещение
площадки выполняется во временном приватном контексте без сохранения cookies,
истории и авторизации между посещениями. Перед разрешённым живым
